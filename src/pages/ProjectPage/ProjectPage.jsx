...
import { getAllProjects, getProject } from "../../redux/projects/projects-operations";
...
const SprintListPage = () => {
  ...
  const projectId = pathname.split('/projects/').join('/sprints').split('/sprints').join('')
          ...
          <Sidebar item="project"/>
           <div className={styles.Sprint}>
            <div className={styles.Header}>
              {isOpenInput ? <ChangeName onClick={openInput}/> : <h2 className={styles.Title}>{projectInfo.name}</h2>}
            ...
            <AddButton variant="member" type="button" text="Добавить людей" className={styles.AddBtn} onClick={openModal}/>
            {showAddPeople && (
            ...
            <ul className={styles.SprintList}>
              {sprintData && sprintData.map((el) => {return (<SprintElement el={el} key={el._id} projectId={projectId}/>)
              })}
